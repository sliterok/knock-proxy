...
// Define a minimal interface for the Greenlock Express instance to satisfy TS
// and allow us to access the hidden `httpsOptions`
...
export async function startServer(opts: ServerOptions) {
    ...
    // 1. INSTANTIATE CHALLENGE STRATEGY
    ...
    // 2. MANAGEMENT PHASE (Greenlock Core)
    // We use the core library to sync your AppConfig allowed hosts into the Greenlock DB.
    ...
    // 3. SERVING PHASE (Greenlock Express)
    // We use the wrapper to get the HTTPS configuration logic
    ...
    }) as unknown as GreenlockInstance; // Cast to access hidden props

    // 4. HIJACK SNI (ClientHello Dropper)
    // We steal the SNI callback generated by Greenlock and wrap it.
    ...
    // 6. START SERVER
    // We use the hijacked options with standard node https
